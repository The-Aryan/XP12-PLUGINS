    ...
    def __init__(self):
        ...
        def StartLogging(self):
            ...
            for name, ref in self.datarefs.items():
                ...
                self.file[0].write("COMM,Sample,Long,Lat,PressureAlt,MagHeading,Pitch,Roll,BaroSet,CAS,TAS,GS,BaroAlt,VSPD,OAT,SLAT,FLAP,LDG\n")
                self.counter[0] = 0
                xp.registerFlightLoopCallback(FlightLoopCallback, self.sampling_rate, 0)
                xp.registerDrawCallback(DrawCallback, xp.Phase_Window)
        ...
        def StopLogging(self):
            if self.file[0]:
                self.file[0].close()
                self.file[0] = None
            xp.unregisterFlightLoopCallback(FlightLoopCallback)
            if self.isDrawing[0]:
                xp.unregisterDrawCallback(DrawCallback, 0)
